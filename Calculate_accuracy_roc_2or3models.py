...
models = ["gap_ResNet50.h5","gap_InceptionV3.h5","gap_Xception.h5"]
...
scaler1 = le_pr.RobustScaler(with_centering = True, with_scaling = True, 
                                quantile_range = (25.0, 75.0))
scaler1.fit(X_train[0,:,:])

scaler2 = le_pr.RobustScaler(with_centering = True, with_scaling = True, 
                                quantile_range = (25.0, 75.0))
scaler2.fit(X_train[1,:,:])

scaler3 = le_pr.RobustScaler(with_centering = True, with_scaling = True, 
                                quantile_range = (25.0, 75.0))
...
trainX[0,:,:] = scaler1.transform(X_train[0,:,:])
trainX[1,:,:] = scaler2.transform(X_train[1,:,:])
trainX[2,:,:] = scaler3.transform(X_train[2,:,:])

testX[0,:,:] = scaler1.transform(X_test[0,:,:])
testX[1,:,:] = scaler2.transform(X_test[1,:,:])
testX[2,:,:] = scaler3.transform(X_test[2,:,:])
...
trainX, y_train_scaled = shuffle(trainX, y_train_scaled)
...
model.fit(trainX, y_train_scaled, batch_size=128, nb_epoch=20, validation_split=0.1)
